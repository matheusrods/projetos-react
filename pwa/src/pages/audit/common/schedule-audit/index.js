...
const ScheduleAudit = ({ Auditing, UserStore }) => {
    const history = useHistory();

    const { programmingInEditing: audit, sourcePage = null, setProgrammingInEditing } = Auditing;
    ...
    const [date, setDate] = useState(audit?.calendarDate || new Date());
    const [startTime, setStartTime] = useState(audit?.startTime || '');
    const [endTime, setEndTime] = useState(audit?.endTime || '');
    const [selectedPeoplesTeam, setSelectedPeoplesTeam] = useState(audit?.auditors || []);
    ...
    const handleSubmit = () => {
        ...
        setProgrammingInEditing(newAudit);
    ...
    const handleSaveTeam = async (items) => {
        ...
        const sameAudit = programacoes.filter((item) => item.codigo !== audit.id);
    ...
    ) : <Redirect to={sourcePage ? `/audit/${sourcePage}` : '/want-to-see'} />;
};

export default inject('Auditing', 'UserStore')(observer(ScheduleAudit));
